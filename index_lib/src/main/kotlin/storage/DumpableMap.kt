...
class DumpableMap<T, U>(private val storageProvider: StorageProvider<T, U> = TempStorageProvider()) :
  SimplifiedMultiMap<T, U> {

  private var storagesCount = 1

  private val lock = ReentrantReadWriteLock()

  internal val storages: MutableMap<Int, HashMap<T, HashSet<U>>> =
    mutableMapOf()
  ...
  private fun storage(key: T): MutableMap<T, HashSet<U>> =
  ...
  private fun storage(index: Int): MutableMap<T, HashSet<U>> {
    while (storages.size > maxStoragesInMemory) {
      val indexToRemove =
        storages.keys.toTypedArray()[Random.nextInt(storages.size)]
  ...
  private fun splitStorages() {
    ...
    repeat(storagesCount) {
      ...
      val newStorageAValues = storageA.filter {
        storageIndex(it.key, newStoragesCount) == storageAIndex
      }
      val newStorageBValues = storageA.filter {
        storageIndex(it.key, newStoragesCount) == storageBIndex
      }
      storageA.clear()
  ...
  private fun storageIndex(key: T, storagesCount: Int = this.storagesCount) =
    key.hashCode().absoluteValue % storagesCount
  ...
  class TempStorageProvider<T, U> : StorageProvider<T, U> {
    ...
    override fun loadStorage(index: Int): HashMap<T, HashSet<U>> {
      ...
      val result =
        FileInputStream(file).use { ObjectInputStream(it).use { it.readObject() } }
    ...
    override fun writeStorage(index: Int, storage: HashMap<T, HashSet<U>>) {
      ...
      FileOutputStream(file).use {
        ObjectOutputStream(it).use {
          it.writeObject(storage)
        }
      }
    }

    private fun storageFile(index: Int): File =
      File(tempDirectory, "cache-$index")
  }

  /**
   * Provider for the persisted data.
   */
  interface StorageProvider<T, U> {
    /**
     * Loads data for the specified index from the persistent storage.
     */
    fun loadStorage(index: Int): HashMap<T, HashSet<U>>

    /**
     * Writes data for the specified index from the persistent storage.
     */
